    ...
    def _request(self, method: str, path: str, payload: dict | None = None,
        ...
        try:
            ...
            try:
                ...
                # Real API nests errors: {"error": {"code": ..., "message": ...}}
                inner = err_json.get("error")
                inner = inner if isinstance(inner, dict) else {}
                code = err_json.get("code") or inner.get("code")
                message = (inner.get("message") or err_json.get("message")
                           or err_body)
            except Exception:
...
class SandboxCursorClient(CursorClient):
    """Subprocess transport for sandboxed environments. EXPERIMENTAL.
    ...
    This contract is unproven against any public ``cursor-agent`` build —
    treat the sandbox transport as experimental. The sandbox broker supplies
    the credential; this transport never sees a key.
    """
    ...
    def cancel_run(self, agent_id: str, run_id: str) -> dict:
        return self._run("cancel", "--agent", agent_id, "--run", run_id)
